                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <div class="bg-light rounded h-100 p-4">
                                <h6 class="mb-4">Course Manage</h6>
                                <button onclick="openLink('create?mode=coursecreate')"><span>Create</span></button>
                                <div class="header-search">
                                    <form action="searchcourse" method="get">
                                        <select class="input-select" name="filter" required>
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            ...
                                            <c:forEach items="${listAllCourse}" var="i">
                                                <tr>
                                                    <th scope="row">${i.getCourse_id()}</th>
                                                    <td>${i.getCategory_name()}</td>
                                                    <td>${i.getCourse_name()}</td>
                                                    <td>${i.getSupplier_name()}</td>
                                                    <td>
                                                        <a style="color: black" href="update?course_id=${i.getCourse_id()}&mode=courseupdate"><i class="fa-sharp fa-solid fa-pen"></i></a>
                                                        <a style="color: black" href="detail?course_id=${i.getCourse_id()}&mode=coursedetail"><i class="fa-solid fa-circle-info"></i></a>
                                                        <a style="color: black" href="delete?course_id=${i.getCourse_id()}&mode=coursedelete"><i class="fa-solid fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                            </c:forEach>   
                                    ...
                                    <c:if test="${endP != 1}">
                                        <c:forEach begin="1" end="${endP}" var="i">
                                            <c:if test="${mode == 'coursemanager'}">
                                            <a class="${index == i? "active":""}" href="coursemanage?index=${i}">${i}</a>
                                            </c:if>
                                            <c:if test="${mode == 'searchcourse'}">
                                            <a class="${index == i? "active":""}" href="searchcourse?index=${i}&filter=${filter}&search=${search}">${i}</a>
                                            </c:if>
                                        </c:forEach>   
                                    </c:if>
                                </div>
