            ...
            <div class="sidebar pe-4 pb-3">
                <nav class="navbar bg-light navbar-light">
                    ...
                    <div class="navbar-nav w-100">
                        ...
                        <a href="ordermanage" class="nav-item nav-link active"><i class="fa fa-table me-2"></i>Order Management</a>             
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <c:forEach items="${listAllOrder}" var="i">
                                                <tr>
                                                    <th scope="row">${i.getOrder_id()}</th>
                                                    <td>${i.getCustomer_name()}</td>
                                                    <td>${i.getOrder_date()}</td>
                                                    <td>
                                                        <select onchange="location.href ='orderchangestatus?order_status='+this.value+'&order_id=${i.getOrder_id()}'">
                                                            
                                                            <option value="1" ${i.getPayment_id() eq '1'?"selected":""}>Approved</option>
                                                            <option value="0" ${i.getPayment_id() eq '0'?"selected":""}>Pending</option>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <a style="color: black" href="orderdetail?order_id=${i.getOrder_id()}&mode=coursedetail"><i class="fa-solid fa-circle-info"></i></a>
                                                    </td>
                                                </tr>
                                            </c:forEach>   
                                        </tbody>
                                    ...
                                    <c:if test="${endP != 1}">
                                        <c:forEach begin="1" end="${endP}" var="i">
                                            <c:if test="${mode == 'ordermanager'}">
                                                <a class="${index == i? "active":""}" href="ordermanage?index=${i}">${i}</a>
                                            </c:if>
                                            <c:if test="${mode == 'searchorder'}">
                                                <a class="${index == i? "active":""}" href="searchorder?index=${i}&filter=${filter}&search=${search}">${i}</a>
                                            </c:if>
                                        </c:forEach>   
                                    </c:if>
                                </div>
